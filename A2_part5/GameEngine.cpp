...
GameEngine::GameEngine() : currentState(new State("start")), warZone(new WarZone()), lastTransitionLog(new std::string("")) {
	transitions = {
...
GameEngine::GameEngine(const GameEngine& other) :
	...
	lastTransitionLog(new std::string(*other.lastTransitionLog)) {
	commandProcessor = new CommandProcessor(this);
}
...
GameEngine& GameEngine::operator=(const GameEngine& other) {
	if (this != &other) {
		...
		*lastTransitionLog = *other.lastTransitionLog;
		transitions = other.transitions;
		delete warZone;
		warZone = new WarZone(*other.warZone);
...
void GameEngine::transition(const std::string& commandType) {
	...
	if (it != transitions.end()) {
		...
		*lastTransitionLog = "[GameEngine] State changed to " + it->second + " using command: " + commandType;
		Notify(this);
